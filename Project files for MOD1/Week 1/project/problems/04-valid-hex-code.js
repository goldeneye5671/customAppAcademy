...
function validHexCode(string) {
    let chars = 'ABCDEF'
    let hexVal = string.split('');
    
    if(hexVal.indexOf('#') !== 0) return false;
    if (hexVal.length !== 7) return false
    
    for(let value of hexVal)
    {
        if (value === "#")
        {
            continue;
        }
        else if (chars.includes(value.toUpperCase()))
        {
            continue;
        }
    ...
    console.log(validHexCode("#222!E4")); // prints false;
    console.log(validHexCode("#22222")); // prints false;
    console.log(validHexCode("#FFFFFF"));
    console.log(validHexCode("#4c5ffc"));
    console.log(validHexCode("#AB$%*^"))
